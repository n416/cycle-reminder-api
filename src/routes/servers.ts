...
const serversRouter = new Hono<HonoEnv>();
...
serversRouter.get('/', protect, async (c) => {
    try {
        ...
        const guilds: any = userDoc.guilds;
        ...
        const serverSettings: { [key: string]: typeof schema.servers.$inferSelect } = {};
        ...
        if (c.env.NODE_ENV === 'development' && userId === '999999999999999999') {
            ...
            if (botGuildsRes.ok) {
                const botGuilds = await botGuildsRes.json() as any[];
...
serversRouter.put('/:serverId/password', protect, async (c) => {
    try {
        ...
        const db = drizzle(c.env.DB, { schema });
        const userDoc = await db.select().from(schema.users).where(eq(schema.users.id, userId)).get();
        const serverInfo = (userDoc?.guilds as any[])?.find((g: any) => g.id === serverId);
...
serversRouter.post('/:serverId/verify-password', protect, async (c) => {
    try {
        ...
        if (!userDoc) return c.json({ message: 'User not found.' }, 404);

        const serverInfo = (userDoc.guilds as any[])?.find((g: any) => g.id === serverId);
        if (!serverInfo) return c.json({ message: 'Forbidden: You are not a member.' }, 403);
