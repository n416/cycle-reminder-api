...
const addLogWithTrim = async (db, logData) => {
...
const calculateNextNotificationInfo = (reminderData, baseTime) => {
...
remindersRouter.post('/:serverId/test-send', protect, protectWrite, async (c) => {
    try {
        ...
        if (finalMessage.includes('{{all}}')) {
            ...
            finalMessage = finalMessage.replace(/\{\{offset\}\}/g, 'まであと 〇〇 分 (テスト)');
        }
...
remindersRouter.put('/:serverId/reorder', protect, protectWrite, async (c) => {
    try {
        ...
        await Promise.all(reminders.map((item) => db.update(schema.reminders).set({ order: item.order }).where(eq(schema.reminders.id, item.id))));
